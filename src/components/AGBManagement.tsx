...
interface SortableAGBItemProps {
  section: TermsAndConditions;
  onEdit: (section: TermsAndConditions) => void;
  onDelete: (id: string) => void;
  onToggleVisibility: (id: string, isActive: boolean) => void;
}

function SortableAGBItem({ section, onEdit, onDelete, onToggleVisibility }: SortableAGBItemProps) {
  const { t } = useLanguage();
...
export function AGBManagement() {
  const { t, language } = useLanguage();
  const [sections, setSections] = useState<TermsAndConditions[]>([]);
  const [loading, setLoading] = useState(true);
  const [editingSection, setEditingSection] = useState<TermsAndConditions | null>(null);
  ...
  function handleEdit(section: TermsAndConditions) {
  ...
  function updateContentItem(lang: Language, index: number, value: string) {
    ...
    const contentKey = `content_${lang}` as keyof TermsAndConditions;
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-2xl font-bold text-white mb-2">{t('admin.agbManagement')}</h2>
          <p className="text-[#9AA0A6]">{t('admin.manageTermsSections')}</p>
        </div>
        <button
          ...
          <div className="bg-[#111316] border border-[#D4AF37]/20 rounded-xl p-8 max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <input
                  type="text"
                  value={editingSection[`heading_${activeLanguageTab}` as keyof TermsAndConditions] as string}
                  onChange={(e) =>
                    setEditingSection({
                      ...editingSection,
              ...
              <div>
                ...
                <div className="space-y-2">
                  {(editingSection[`content_${activeLanguageTab}` as keyof TermsAndConditions] as string[]).map((item, index) => (
